...
class ReferenceTwoPhoneNumberInputField extends HookWidget {
  const ReferenceTwoPhoneNumberInputField({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<MiscellaneousDetailsFormBloc,
      ...
      listener: (context, state) {
        if (!state.isEditing) {
          controller.text = state.referenceDetails.referenceTwoPhoneNumber.value
              .getOrElse(() => "");
        }
      },
      ...
      builder: (context, state) {
        if (state.isEditing &&
            state.referenceDetails.referenceTwoPhoneNumber.isValid()) {
          controller.text = state.referenceDetails.referenceTwoPhoneNumber.value
              .getOrElse(() => "");
        }
        return TextFormField(
          ...
          onChanged: (referenceTwoPhoneNumber) => context
              .read<MiscellaneousDetailsFormBloc>()
              .add(MiscellaneousDetailsFormEvent.referenceTwoPhoneNumberChanged(
                referenceTwoPhoneNumber,
              )),
        );
      },
